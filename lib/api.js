...
export const api = {
  ...
  deleteTournament: (id, token) => request(`/tournaments/${id}`, { method: 'DELETE', token }),
  ...
  addResultFile: (tournamentId, payload, token) =>
    request(`/tournaments/${tournamentId}/result-files`, { method: 'POST', body: payload, token }),
  removeResultFile: (tournamentId, fileId, token) =>
    request(`/tournaments/${tournamentId}/result-files/${fileId}`, { method: 'DELETE', token }),

  adminListFiles: (token) => request('/upload/files', { token }),
  adminDeleteFile: (filename, token) => request(`/upload/files/${filename}`, { method: 'DELETE', token }),
  ...
  getMyTeams: (token) => request('/teams/my-teams', { token }),
